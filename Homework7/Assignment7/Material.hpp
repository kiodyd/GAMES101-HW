...
private:
    ...
    // MicroFacet BRDF
    float DistributionGGX(Vector3f N, Vector3f H, float a)
    {
        float a2     = a*a;
        float NdotH  = fmax(dotProduct(N, H), 0.0);
    ...
    {
        float NdotV = fmax(dotProduct(N, V), 0.0);
        float NdotL = fmax(dotProduct(N, L), 0.0);
